...
from laptopdeals.history import load_history, latest_price, stats
...
from laptopdeals.sources import lenovo
from laptopdeals.specs import clean_text, parse_display_psref, parse_spec_codes
from laptopdeals.timeutil import iso_date
from laptopdeals import psref

from laptopdeals.catalog import LIVE_FIELDS, existing_ids_from, is_display_name, model_name
    ...
    **kwargs: Any,
    ...
    for _, rows in (groups or {}).items():
        ...
        for product in rows:
            ...
            if "CTO" in sku or cto_path.exists():
                row["is_cto"] = True
                row["custom_model"] = True
                if cto_path.exists():
                    payload = read_json(cto_path, {})
                    row["cto_options"] = {key: value for key, value in payload.items() if key != "lastFetched"}
                    def_cto = psref.extract_default_cto_specs(payload)
                    row_specs = row.setdefault("tech_specs", {})
...
from laptopdeals.providers import register  # noqa: E402

register("lenovo", sys.modules[__name__])
