...
from laptopdeals.datafile import load_data, write_data
...
def main() -> int:
    ...
    common_sub.add_argument("--data", default=argparse.SUPPRESS)
    common_sub.add_argument("--history-dir", default=argparse.SUPPRESS)

    parser = argparse.ArgumentParser(description="Price history and live price operations", parents=[common])
    sub = parser.add_subparsers(dest="command", required=True)

    lenovo_parser = sub.add_parser("lenovo-current", parents=[common_sub], help="Fetch current Lenovo price and append only changed price points")
    ...
    refurb_parser = sub.add_parser("refurb-current", parents=[common_sub], help="Fetch current Lenovo Outlet refurb prices & inventory and append only changed price points")
    add_targets(refurb_parser)
    refurb_parser.add_argument("--workers", type=int, default=4)
    refurb_parser.add_argument("--delay-min", type=float, default=0.5)
    refurb_parser.add_argument("--delay-max", type=float, default=1.5)
    refurb_parser.add_argument("--dry-run", action="store_true")

    bitbns_parser = sub.add_parser("bitbns-history", parents=[common_sub], help="Fetch BitBns history for new listings or existing products")
    ...
    bitbns_parser.add_argument("--only-new", "--missing-history-only", dest="only_new", action="store_true", help="Only fetch products without a local history file.")
    ...
    stats_parser = sub.add_parser("stats", parents=[common_sub], help="Recalculate stats from local history")
    add_targets(stats_parser)
    stats_parser.add_argument("--dry-run", action="store_true")

    compress_parser = sub.add_parser("compress", parents=[common_sub], help="Normalize history files to change-points")
    ...
    if args.command == "lenovo-current":
    ...
    elif args.command == "refurb-current":
        ...
        result = pricing.update_from_bitbns(
            ...
            ids=ids,
            mode=mode,
            missing_history_only=args.only_new,
            workers=args.workers,
            delay=args.delay,
            dry_run=args.dry_run,
        )
